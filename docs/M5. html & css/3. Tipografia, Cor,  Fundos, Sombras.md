# 3. Tipografia, Cor, Fundos, Sombras
## Tipografia
...
| rem | semelhante ao "em", mas o tamanho é sempre relativo ao tamanho base (`html` é o elemento root , normalmente 16px)  |
...
Uma boa prática será usar uma base  no `<html>` e depois **rem** ou **%** nos elementos.
...
Neste exemplo estamos a formatar a cor de fundo a branco mas com uma transparência de 40%, para isso usámos a função `rgba`.
...
> [!NOTE] HSL
> O formato *Hue, Saturation and Light* é normalmente usado quando se pretende manipular a cor do elemento, permitindo uma forma fácil de escurecer ou clarear uma cor, alterando apenas o parâmetro da luz. Por outro lado também é fácil encontrar cores complementares ajustando apenas o parâmetro *Hue*.
> ```css
> color: hsl(200, 80%, 50%);     /* Base blue */
color: hsl(200, 80%, 30%);     /* Darker blue */
color: hsl(200, 80%, 70%);     /* Lighter blue */
>
> color: hsl(120, 100%, 50%);    /* Green */
>color: hsl(300, 100%, 50%);    /* Complementary: Magenta (120° + 180°) */
>```
...
> [!Tip] Fundo Fixo
> Com a propriedade `background-attachment: fixed;` a imagem de fundo do elemento fica fixa, ou seja, quando se efetua scroll a secções seguintes começam a subir a acabam por "passar por cima" deste fundo, criando um efeito de cobrir ou descobrir uma imagem. 



### Blend mode

Por vezes há a necessidade de clarear ou de escurecer a imagem de fundo para se conseguir uma melhor leitura do que está por cima, tipicamente um bloco de texto. No exemplo seguinte apresentamos um exemplo de clarear a imagem de fundo. o blend mode `multiply`também é muito utilizado.
...
Também é possível aplicar um gradiente a um parágrafo ou heading. Neste caso o texto vai refletir o gradiente através de um mecanismo de clipping sobre o fundo.
...
Mais sobre [gradientes](https://web.dev/learn/css/gradients) ou cria o teu gradiente [aqui](https://cssgradient.io/).
...
|---|---|
|`offset x`|Defasamento horizontal da sombra relativamente à posição do elemento (canto superior esquerdo).|
|`offset y`|Defasamento vertical da sombra relativamente à posição do elemento (canto superior esquerdo).|
|`blur`|Quantidade de esbatimento (desfoque) aplicado à sombra.|
|`spread radius` (opcional)|Quantidade de expansão da sombra para além da borda do elemento.|
