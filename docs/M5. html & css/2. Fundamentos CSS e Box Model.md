...
Por ex.
```css
/* Formata link independentemente do estado */
a {
  font-weight: bold;
}
...
## Variáveis css
Atualmente existe a possibilidade de criar variáveis em css, também conhecidas como *custom css properties*. As varáveis têm a vantagem de ser definidas num local apenas e utilizadas em múltiplos locais do nosso css. São utilizadas para definir parâmetros estruturais do site, tais como a fonte utilizada, a cor primária e secundária.

As variáveis são definidas num determinado selector e ficam disponíveis no âmbito desse selector e de todos os seus descendentes. São normalmente definidas ao nível mais alto o `:root`uma pseudo-class que se refere ao elemento `html`.

```css
:root {
	--primary-color: #033268;
	--secondary-color: #e2ece3;
}
