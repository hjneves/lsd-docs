...
A flexbox é um modelo de layout CSS que é muito usado para estruturar uma página html, sendo a base de muitas frameworks css. Tem por base um elemento com `display: flex;` e uma série de elementos filhos diretos que se organizam mediante a configuração da flexbox.
...
Existem normalmente duas abordagens para o responsive:
- *mobile first*: o site é desenvolvido tendo por base o layout de mobile e depois são criadas as adaptações necessárias para o ambiente desktop.
- *desktop first*: neste caso o site é desenvolvido para o ambiente desktop e depois são criadas as medias queries que adaptam o site para mobile. 
...
O menu em desktop é normalmente apresentado numa barra superior e tem por base um conjunto de items. Já em mobile por uma questão de usabilidade e de espaço é normalmente apresentado um icon de menu hamburger que abre então uma lista com os items do menu.
Conforme a complexidade do menu podemos ter eventualmente a necessidade de criar um conjunto de items do menu diferente para mobile e desktop.
### Layout
...
Usando a propriedade  `prefers-color-scheme `temos a possibilidade de adaptar uma página para os ambientes `light` e `dark`  que o utilizador escolheu no sistema operativo do seu equipamento.
...
Pegando nas duas estratégias principais para assegurar a qualidade responsive de uma página verificamos que em :
 - **mobile based** - todo o CSS de uma página está pensado para a versão mobile da página e as media queries vão adaptando o site  aos ecrãs maiores.
 - **desktop based** - o CSS do site é pensado para a versão base em desktop, e depois as media queries vão adaptando a página para os ecrãs mais pequenos.

Em termos de media queries temos tipicamente as seguintes condições:
...
Contendo as media queries css de alteração ao css geral da página, estas deverão vir sempre no final. 
- Se incluirmos as media queries no `styles.css` deverão ser colocadas no fim do código css geral.
