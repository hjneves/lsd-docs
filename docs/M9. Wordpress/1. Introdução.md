...
Deixo aqui duas abordagens para a instalação local de instâncias de wordpress. A primeira baseado numa stack exclusiva para a gestão de sistes wordpress e outra mais generalista para desenvolvimento web.
### Stack Wordpress

A aplicação [LocalWP](https://localwp.com/) permite a criação de um ou mais sistes wordpress a partir de uma aplicação que faz instalação e gestão integradas do wordpress.
Podem ser criados vários projetos e definidas definições ao nivel de versões para cada um deles.


### Stack MAMP

A stack [MAMP](https://www.mamp.one/) é usada para o desenvolvimento web de forma generalista. Assim, o wordpress tem de ser instalado a partir do site wordpress.org através dos seguintes passos:
...
A melhor forma de migrar o site da máquina local de desenvolvimento para o alojamento é através de um plugin de migração.
