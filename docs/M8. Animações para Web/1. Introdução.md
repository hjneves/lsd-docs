As animações em web pode ser definidas em CSS, Javascript ou no elemento `<canvas>`.
O foco neste módulo serão as animações basedas em CSS e alguns plugins JS que desencadeiam animações via javascript (GSAP)
